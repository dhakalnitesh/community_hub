...
{
    public function index(Grievance $grievance)
    {
        abort_if($grievance->hidden_at, 404);

        $comments = GrievanceComment::with(['user', 'replies.user'])
    ...
    public function store(Grievance $grievance, Request $request)
    {
        abort_if($grievance->hidden_at, 404);
        ...
        $validated = $request->validate([
            ...
            'parent_id' => [
                'nullable',
                'exists:grievance_comments,id',
                function ($attribute, $value, $fail) use ($grievance) {
                    if ($value && !GrievanceComment::where('id', $value)->where('grievance_id', $grievance->id)->exists()) {
                        $fail('The parent comment must belong to the same grievance.');
                    }
                },
            ],
        ]);

        $comment = GrievanceComment::create([
    ...
    public function destroy(GrievanceComment $comment, Request $request)
    {
        $user = $request->user();
