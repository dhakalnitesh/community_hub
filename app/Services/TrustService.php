...
class TrustService
{
    ...
    private function calculateUserScore(User $user): float
    {
        ...
        $submissions = Grievance::where('reporter_ip_hash', $user->id)
            ->count();
        ...
        $spamHits = Grievance::where('assigned_to', $user->id)
    ...
    public function getEffectivePriority(Grievance $grievance): string
    {
        $priority = $grievance->user_priority ?? $grievance->priority;
        $levels = ['low' => 0, 'medium' => 1, 'high' => 2, 'critical' => 3];
        $level = $levels[$priority] ?? 1;

        $trustScore = $this->getScore(null, $grievance->anonymous_uuid);
        ...
        $mergedCount = Grievance::where('duplicate_of_id', $grievance->id)->count();
        if ($mergedCount >= 5) {
            $level = min($level + 1, 3);
        }
